/*
  ...
  1. New Tables
    - `sa_government_verifications` - Main verification tracking
    - `sa_identity_verifications` - Home Affairs ID verification
    - `sa_education_verifications` - SAQA education verification
    - `sa_employment_verifications` - SARS/DoL employment verification
    - `sa_professional_licenses` - Professional council registrations
    - `sa_seta_certifications` - SETA skills development tracking
    - `sa_bee_verifications` - BEE compliance verification

  2. Enums
    - South African specific verification types and agencies
    - Provincial and language enums
    - BEE level classifications

  3. Security
    - Enable RLS on all tables
    - Add policies for user access and recruiter visibility
    - Service role management access

  4. Views and Functions
    - `sa_reelpass_status` view for comprehensive status
    - `calculate_sa_reelpass_score` function for scoring
*/
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'profiles' AND column_name = 'sa_id_number'
  ) THEN
    ALTER TABLE profiles ADD COLUMN sa_id_number text;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE profiles ADD COLUMN province sa_province;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE profiles ADD COLUMN languages sa_language[];
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'profiles' AND column_name = 'bee_status'
  ) THEN
    ALTER TABLE profiles ADD COLUMN bee_status bee_level;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'profiles' AND column_name = 'work_permit_status'
  ) THEN
    ALTER TABLE profiles ADD COLUMN work_permit_status text;
  END IF;
END $$;
...
CREATE POLICY "Service role can manage all SA education verifications"
  ON sa_education_verifications
  FOR ALL
  TO service_role
  USING (true)
...
CREATE POLICY "Service role can manage all SA employment verifications"
  ON sa_employment_verifications
  FOR ALL
  TO service_role
  USING (true)
...
CREATE POLICY "Service role can manage all SA professional licenses"
  ON sa_professional_licenses
  FOR ALL
  TO service_role
  USING (true)
...
CREATE POLICY "Service role can manage all SA SETA certifications"
  ON sa_seta_certifications
  FOR ALL
  TO service_role
  USING (true)
...
CREATE POLICY "Service role can manage all SA BEE verifications"
  ON sa_bee_verifications
  FOR ALL
  TO service_role
  USING (true)
