/*
  # Add Core ReelHunter Features

  1. New Tables
    - `recruiter_scorecards` - Public ratings for recruiters
    - `candidate_communications` - Track automated communications
    - `availability_updates` - Live availability tracking
    - `pipeline_stages` - Customizable pipeline stages
    - `candidate_pipeline_positions` - Track candidates in pipeline

  2. Updates
    - Add availability tracking to profiles
    - Add communication preferences
    - Add recruiter rating system

  3. Functions
    - Auto-communication triggers
...
CREATE POLICY "Candidates can create scorecards"
  ...
  WITH CHECK (
    EXISTS (
      ...
      WHERE id = candidate_id AND user_id = uid() AND role = 'candidate'
    )
  );
...
CREATE POLICY "Users can read own scorecards"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM profiles 
      WHERE (id = recruiter_id OR id = candidate_id) AND user_id = uid()
    )
  );
...
CREATE POLICY "Users can manage own availability"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM profiles 
      WHERE id = profile_id AND user_id = uid()
    )
  );
...
CREATE POLICY "Recruiters can read candidate availability"
  ...
  USING (
    ...
    EXISTS (
      ...
      WHERE recruiter.user_id = uid() AND recruiter.role = 'recruiter'
    ) AND
    EXISTS (
...
CREATE POLICY "Recruiters can manage own pipeline stages"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM profiles 
      WHERE id = recruiter_id AND user_id = uid() AND role = 'recruiter'
    )
  );
